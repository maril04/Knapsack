...
namespace Knapsack
{
    public partial class option_with_cost : Form
    {
        ...
        private bool ValidateInputs(out int maxCapacity)
        {
            ...
            if (!int.TryParse(textBox1.Text, out maxCapacity) || maxCapacity <= 0 || maxCapacity > 1000000)
            {
                ShowError("Вес рюкзака должен быть целым числом от 1 до 1 000 000!", textBox1);
                return false;
            }
            ...
            for (int i = 1; i < dataGridView1.RowCount; i++)
            {
                for (int j = 1; j < dataGridView1.ColumnCount; j++)
                {
                    ...
                    if (!int.TryParse(cellValue, out var parsedValue) || parsedValue <= 0 || parsedValue > 1000000000)
                    {
                        ShowError("Введите корректные положительные числа (не более 1 000 000 000) в таблице!", dataGridView1);
                        return false;
                    }
                }
            }
